...
?>
<html>
<head><title>Create New Project</title>
...
<?php
	...
	$id = $_SESSION['id']; 
	$query = "SELECT id, CONCAT (Fname, ' ' , Lname) AS 'Client Name' FROM users WHERE id = '$id'";
	$result = @mysqli_query($conn, $query);
	$row = mysqli_fetch_array($result);
		if($row)
		{		
			echo "<label>Client Name: </label>" . "<b>". $row[1] . "</b><br><br>";
		}
		mysqli_close($conn);
?>
<?php
...
session_start();
	...
	$result = @mysqli_query($conn, $query);
		header("Location: createsave.php");
		mysqli_close($conn);
	}
}
?>
	...
	<center><input style="float:left; margin-left: 270px;" type="submit" class = "block" name="btnSubmit" value="Submit"><br>
	<center><br><a href="javascript:history.back()">Cancel</a><br><br>
